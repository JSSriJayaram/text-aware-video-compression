...
h265_compressor.py
==================
H.265 / HEVC compression via ffmpeg libx265.

target_ssim → CRF mapping (H.265 achieves same quality at ~6 lower CRF):
  ssim 1.0  →  CRF 12
  ssim 0.90 →  CRF 20
  ssim 0.80 →  CRF 27
  ssim 0.70 →  CRF 33
...
device:
  "mps" / "videotoolbox"  → ffmpeg -c:v hevc_videotoolbox (Apple HW)
  "cpu"                   → ffmpeg -c:v libx265 (software, slower)
"""
...
def _ssim_to_crf_h265(target_ssim: float) -> int:
    t   = float(np.clip(target_ssim, 0.5, 1.0))
    crf = 51 - int((t - 0.5) / 0.5 * 39)   # 0.5→51,  1.0→12
...
class H265Compressor(BaseCompressor):
    NAME = "h265"
    ...
    def compress(
        ...
        print(f"  [H265/{dev_label.upper()}] target_ssim={target_ssim:.2f} → CRF={crf}")
        t0  = time.time()
